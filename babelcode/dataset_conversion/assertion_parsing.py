...
"""Functions for parsing schema and literals from assertion statements."""
...
class LiteralParser(ast.NodeVisitor):
  ...
  def validate_types(self, schema_types: List[str]) -> str:
    ...
    numeric_types_found = False

    for v in schema_types:
      for t in ['long', 'integer', 'float', 'double']:
        if t in v:
          numeric_types_found = True
          break

    if numeric_types_found:
  ...
  def visit_Dict(self, node: ast.Dict) -> None:
    ...
    if key_type not in ['integer', 'string', 'boolean']:
