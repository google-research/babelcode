...
def consolidate_schema_from_test_cases(
  ...
  for tc_id, tc in test_cases.items():
    ...
    expected_schema = _consolidate_type(
        'expected', potential_expected_type, expected_schema
    )
...
def get_arguments_from_solution(qid, solution, entry_fn_name):
  ...
  for arg in argument_nodes:
    ...
    if arg_name in arg_types:
      logging.error('Found duplicate argument for %s in %s', arg_name, qid)
      logging.error(arg_types)
      raise data_types.QuestionValidationError('Duplicate Arg Name')
