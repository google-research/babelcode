...
class TestLanguageExecution(testing_utils.BaseLanguageTestingClass):
  ...
  def make_executable_code(
    ...
    return_value = self.literal_translator.convert_var_to_literal(
        schema['expected'], code_return_value
    )
  ...
  def test_map_equality(self, lang_name, is_correct, tmp_path):
    ...
    question = Question(
        ...
        schema=[
            ...
            {'name': 'expected', 'type': 'map<string;list<double>>'},
        ],
        test_list=[
  ...
  def test_list_equality(self, lang_name, is_correct, tmp_path):
    ...
    question = Question(
        ...
        schema=[
            {'name': 'arg0', 'type': 'boolean'},
            {'name': 'expected', 'type': 'list<map<string;boolean>>'},
        ],
        test_list=[
  ...
  def test_set_equality(self, lang_name, is_correct, tmp_path):
    ...
    question = Question(
        ...
        title='testing',
        schema=[
            {'name': 'arg0', 'type': 'set<string>'},
            {'name': 'expected', 'type': 'set<integer>'},
        ],
        test_list=[
  ...
  def test_null_equality(self, lang_name, type_str, tmp_path):
    ...
    question = Question(
        ...
        title='testing',
        schema=[
            {'name': 'arg0', 'type': type_str},
            {'name': 'expected', 'type': type_str},
        ],
        test_list=[
  ...
  def test_primitive_equality(self, lang_name, is_correct, tmp_path):
    self._setup_test(lang_name)
    ...
    question = Question(
        ...
        test_list=[
            dict(idx=0, inputs={'arg0': True}, outputs='Correct'),
        ],
        entry_fn_name='test',
    )

    code_path, pred_code = self.make_executable_code(
        tmp_path, question, 'Correct' if is_correct else 'Wrong'
    )
    result = execute_code(
        prediction=Prediction('1', '0', lang_name, pred_code, code_path),
        commands=self.lang.command_fn(code_path),
