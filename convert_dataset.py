...
class RawQuestion:
  """Dataclass used as a way to validate that specific keys present in each line of the jsonl input.

  Attributes:
    id: The id of the question.
    title: The title of the question.
    testing_code: The testing code to check a solution.
    entry_fn_name: The name of the solution function.
    solution: The solution code.
    text: The text description of the question.
    entry_cls_name: The name of the class the solution belongs to (optional).
    metadata: Any metadata about the question itself.
  """
...
def convert_raw_questions(
    read_quesitons: Dict[str, Dict[str, Any]], fixes: Dict[str, Dict[str, Any]]
):
  """Converts a dictionary of raw questions into the parsed form.

  Args:
    read_quesitons: The map of question id to raw question object.
  ...
  raw_question_map = {}
  for qid, raw_question in read_quesitons.items():
    fix = fixes.pop(qid, {})
  ...
  for qid, raw_question in raw_question_map.items():
    ...
    gold_prediction = {
        'qid': raw_question.id,
        'id': 0,
        'code': raw_question.solution.replace(old_entry_point, new_entry_point),
        'language': 'Python',
        'testing_code': raw_question.testing_code.replace(
            old_entry_point, new_entry_point
        ),
        'text': raw_question.text,
    }
    parsed_questions[raw_question.id] = (parsed_question, gold_prediction)
...
def convert_dataset(
  ...
  if debug_question:
    logging.warning(f'Only parsing {debug_question=}')
  ...
  with gold_path.open('w') as gold_file:
    with output_path.open('w') as parsed_file:
      for qid, (parsed, gold) in parsed_questions.items():
        ...
        gold_file.write(f'{json.dumps(gold)}\n')
