...
def execute_predictions_from_file(experiment_name: str,
  ...
  if eval_language:
    if eval_language not in langs_found:
      raise ValueError(f'{eval_language=} not found in predictions')
    logging.warning('Only evaluating %s', eval_language)
    langs_found = [eval_language]
...
if __name__ == '__main__':
  ...
  _LANGUAGE = flags.DEFINE_string('language', None,
                                  'If passed, only evaluate passed language.')
